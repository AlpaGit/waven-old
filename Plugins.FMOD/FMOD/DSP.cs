...
namespace FMOD
{
  public struct DSP
  {
    ...
    public RESULT setParameterData(int index, byte[] data) => DSP.FMOD5_DSP_SetParameterData(this.handle, index, Marshal.UnsafeAddrOfPinnedArrayElement<byte>(data, 0), (uint) data.Length);
    ...
    public RESULT getCPUUsage(out uint exclusive, out uint inclusive) => DSP.FMOD5_DSP_GetCPUUsage(this.handle, out exclusive, out inclusive);
