...
// Type: Ankama.Cube.SRP.RenderOpaquePass
...
namespace Ankama.Cube.SRP
{
  public class RenderOpaquePass : AbstractPass
  {
      ...
      ForwardRenderer renderer)
    {
      ...
      CommandBuffer commandBuffer = CommandBufferPool.Get("Rendre Opaques");
      using (new ProfilingSample(commandBuffer, "Rendre Opaques", (CustomSampler) null))
      {
        SRPUtility.SetRenderTarget(commandBuffer, this.m_colorRTI, this.m_depthRTI);
        context.ExecuteCommandBuffer(commandBuffer);
        commandBuffer.Clear();
        DrawRendererSettings rendererSettings = this.CreateDrawRendererSettings(renderingData.camera, SortFlags.CommonOpaque, renderingData.renderConfiguration, qualitySettings.dynamicBatching);
