...
// Type: Ankama.Cube.Fight.Entities.SummoningStatus
...
namespace Ankama.Cube.Fight.Entities
{
  public sealed class SummoningStatus : CharacterStatus
  {
    ...
    [NotNull]
    public static SummoningStatus Create(
      int id,
      [NotNull] SummoningDefinition definition,
      int level,
      PlayerStatus owner,
      Vector2Int position)
    {
      ...
      SummoningStatus status = new SummoningStatus(id, id1, teamId, teamIndex, level);
      status.area = area;
