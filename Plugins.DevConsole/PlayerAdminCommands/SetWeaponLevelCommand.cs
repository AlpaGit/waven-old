...
// Type: DevConsole.PlayerAdminCommands.SetWeaponLevelCommand
...
{
  [UsedImplicitly]
  [ConsoleCommand(new string[] {"player/setWeaponLevel"})]
  internal class SetWeaponLevelCommand
  {
    public static string Help(string command, bool verbose) => "Set the level of one weapon. Parameters : <id> <level>";
    ...
    public static AdminCmd NetworkCommand(string[] tokens)
    {
      ...
      int result2 = 0;
      if (tokens.Length >= 2)
        int.TryParse(tokens[1], out result2);
      return new AdminCmd()
      {
        SetWeaponLevel = new AdminCmd.Types.SetLevel()
        {
          Id = result1,
          Level = result2
        }
